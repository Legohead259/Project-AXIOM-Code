...
//    digitalWrite(LED_BUILTIN, armed); // Use the built-in LED to indicate the vehicle's armed status. ON = ARMED, OFF = DISARMED
    ...
    else if (pitch_value < JOYSTICK_IDLE - THRESHOLD && armed) { // Check if the Y-Axis is pushed down and vehicle armed
    ...
    int yaw_value = analogRead(JOYSTICK_X_PIN);
    ...
    else if (yaw_value < JOYSTICK_IDLE - THRESHOLD && armed) { // Check if the X-Axis is pushed right and vehicle armed
